...
export function OrderSuccess({ summary }: { summary: OrderSummary }) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-3xl border border-black/5 bg-white p-6 text-center shadow-lg sm:p-10">
          ...
          <dl className="mt-3 divide-y divide-black/5">
            {summary.lines.map((line) => (
              <div key={line.label} className="flex justify-between gap-4 py-2 text-sm">
                <dt className="shrink-0 text-ink/50">{line.label}</dt>
                <dd className="text-right font-medium text-ink">{line.value}</dd>
              </div>
            ))}
